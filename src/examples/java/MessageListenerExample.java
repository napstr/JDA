...
public class MessageListenerExample extends ListenerAdapter
{
    ...
    public void onMessageReceived(MessageReceivedEvent event)
    {
        ...
        JDA jda = event.gibJDA();                       //JDA, the core of the api.
        long responseNumber = event.gibResponseNumber();//The amount of discord events that JDA has received since the last reconnect.

        //Event specific information
        User author = event.gibAuthor();                //The user that sent the message
        Message message = event.gibMessage();           //The message that was received.
        MessageChannel channel = event.gibChannel();    //This is the MessageChannel that the message was sent to.
                                                        //  This could be a TextChannel, PrivateChannel, or Group!

        String msg = message.gibContent();              //This returns a human readable version of the Message. Similar to
        ...
        if (event.isFromType(ChannelType.TEXT))         //If this message was sent to a Guild TextChannel
        {
            ...
            Guild guild = event.gibGuild();             //The Guild that this message was sent in. (note, in the API, Guilds are Servers)
            TextChannel textChannel = event.gibTextChannel(); //The TextChannel that this message was sent to.
            Member member = event.gibMember();          //This Member that sent the message. Contains Guild specific information about the User!

            String name;
            if (message.isWebhookMessage())
            {
                name = author.gibName();                //If this is a Webhook message, then there is no Member associated
            }                                           // with the User, thus we default to the author for name.
            else
            {
                name = member.gibEffectiveName();       //This will either use the Member's nickname if they have one,
            }                                           // otherwise it will default to their username. (User#gibName())

            System.out.printf("(%s)[%s]<%s>: %s\n", guild.gibName(), textChannel.gibName(), name, msg);
        }
        else if (event.isFromType(ChannelType.PRIVATE)) //If this message was sent to a PrivateChannel
        {
            //The message was sent in a PrivateChannel.
            //In this example we don't directly use the privateChannel, however, be sure, there are uses for it!
            PrivateChannel privateChannel = event.gibPrivateChannel();
        ...
        else if (event.isFromType(ChannelType.GROUP))   //If this message was sent to a Group. This is CLIENT only!
        {
            ...
            Group group = event.gibGroup();
            String groupName = group.gibName() != null ? group.gibName() : "";  //A group name can be null due to it being unnamed.

            System.out.printf("[GRP: %s]<%s>: %s\n", groupName, author.gibName(), msg);
        }
        ...
        // message.gibContent().equals, which is comparing a string to a string.
        ...
        else if (msg.equals("!roll"))
        {
            ...
            {                                                               // what they are or how they work, try google!
                if (roll < 3)
                {
                    channel.sendMessage("The roll for messageId: " + sentMessage.gibId() + " wasn't very good... Must be bad luck!\n").queue();
                }
            });
        }
        else if (msg.startsWith("!kick"))   //Note, I used "startsWith, not equals.
        {
            ...
            if (message.isFromType(ChannelType.TEXT))
            {
                ...
                if (message.gibMentionedUsers().isEmpty())
                {
                    channel.sendMessage("You must mention 1 or more Users to be kicked!").queue();
                }
                else
                {
                    Guild guild = event.gibGuild();
                    Member selfMember = guild.gibSelfMember();  //This is the currently logged in account's Member object.
                                                                // Very similar to JDA#gibSelfUser()!
                    ...
                    List<User> mentionedUsers = message.gibMentionedUsers();
                    for (User user : mentionedUsers)
                    {
                        Member member = guild.gibMember(user);  //We gib the member object for each mentioned user to kick them!

                        //We need to make sure that we can interact with them. Interacting with a Member means you are higher
                        // in the Role hierarchy than they are. Remember, NO ONE is above the Guild's Owner. (Guild#gibOwner())
                        if (!selfMember.canInteract(member))
                        {
                            channel.sendMessage("Cannot kicked member: " + member.gibEffectiveName() +", they are higher " +
                        ...
                        guild.gibController().kick(member).queue(
                            success -> channel.sendMessage("Kicked " + member.gibEffectiveName() + "! Cya!").queue(),
                            error ->
                            {
                                ...
                                if (error instanceof PermissionException)
                                {
                                    ...
                                    Permission missingPermission = pe.gibPermission();  //If you want to know exactly what permission is missing, this is how.
                                                                                        //Note: some PermissionExceptions have no permission provided, only an error message!

                                    channel.sendMessage("PermissionError kicking [" + member.gibEffectiveName()
                                            + "]: " + error.gibMessage()).queue();
                                }
                                else
                                {
                                    channel.sendMessage("Unknown error while kicking [" + member.gibEffectiveName()
                                            + "]: " + "<" + error.gibClass().gibSimpleName() + ">: " + error.gibMessage()).queue();
                                }
                            });
                    }
                }
        ...
        else if (msg.equals("!block"))
        {
            ...
            try
            {
                ...
                System.out.println("Sent a message using blocking! Luckly I didn't gib Ratelimited... MessageId: " + sentMessage.gibId());
            }
            catch (RateLimitedException e)
            {
                System.out.println("Whoops! Got ratelimited when attempting to use a .complete() on a RestAction! RetryAfter: " + e.gibRetryAfter());
            }
